...
#@profile
def UpdateMembershipMatrix(DataMatrix, H, S, Centroids, MembershipMatrix, RegParam, GeneralSolver):
    GetHMatrix(DataMatrix, H, S, Centroids)

    #print DataMatrix

    #minimization solving
    #This part essentially corresponds to formula 1
    if GeneralSolver == 1:
        ...
        out = "" + str(NumRows) + "," + str(NumFeatures) + "," + str(StrictRegParam) + ","

        for value in H_Flat:
            out += str(value) + ","


        with open('interface_file.csv', 'w') as file:
            file.write(out)
        ...
        while True:
            ...
            if output == '' and process.poll() is not None:
                break
            if output:
                MM_Flat = np.fromstring(output, dtype=np.float64, sep=",")
...
def RSFKM(DataMatrix, KClusters, RegParam, ThresholdValue, OutputDirectory, GeneralSolver):
    #variables
    ...
    while not Convergence(OldCentrioids, Centroids, TimeStep):
        ...
        UpdateMembershipMatrix(DataMatrix, MatrixH, S, Centroids, MembershipMatrix, RegParam, GeneralSolver)
        Centroids = FindCentroids(DataMatrix, Centroids, S, MembershipMatrix)
